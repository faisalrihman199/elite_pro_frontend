...
const Sidebar = () => {
  ...
  return (
    <div>
      ...
      <aside
        id="logo-sidebar"
        className={`fixed top-0 left-0 z-40 w-64 h-screen z-10 transition-transform ${isOpen ? 'translate-x-0' : '-translate-x-full'
          } sm:translate-x-0`}
        aria-label="Sidebar"
      >
        <div className="h-full px-3 py-4 overflow-y-auto" style={{
          ...
          <ul className="space-y-2 font-medium">
            <li>
              <Link
                to={`${isAdmin ? '/dashboard' : '/dashboard/one_employee'}`}
                className={`flex items-center p-2 rounded-lg text-white hover:bg-gray-700 group ${isActiveLink('/dashboard') || isActiveLink('/one_employe') ? 'active-link' : ''
                  }`}
              >
